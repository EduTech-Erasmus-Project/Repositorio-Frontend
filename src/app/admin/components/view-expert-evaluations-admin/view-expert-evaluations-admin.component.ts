        ...
        element.conceptEvaluations.forEach((element1) => {
          element1.questionEvaluations.forEach((element2) => {
            this.angForm2.addControl(
              element2.questionid,
              new FormControl({value:element2.qualification, disabled: true}, [Validators.required])
            );
          });
        });
        this.angForm2.addControl(
          "observation",
          new FormControl({value:element.observation , disabled: true}, [Validators.required])
        );
      });
