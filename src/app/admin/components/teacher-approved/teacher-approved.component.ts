...
export class TeacherApprovedComponent implements OnInit {
  ...
  constructor(
    ...
    this.breadcrumbService.setItems([
      {
        label: "Docentes y expertos aprobados",
        routerLink: ["/admin/teacher/request/approved"],
      },
    ]);
  }
  ...
  getProfile(id: number) {
    this.router.navigate([
      `/admin/teacher/request/approved/teacher-expert-profile/${id}/approved`,
    ]);
  }
  getLearningObjectEvaluatedByExpert(id: number) {
    this.router.navigate([
      `/admin/teacher/request/approved/learning-object/evaluated/${id}`,
    ]);
  }
  getLearningObjectUploadByTeacher(id: number) {
    this.router.navigate([
      `/admin/teacher/request/approved/learning-object/upload/${id}`,
    ]);
  }
  confirm2(
    event: Event,
    ...
    this.confirmationService.confirm({
      ...
      accept: () => {
        this.administratorService
          .updateTeacherAproved(id, teacher_status, expert_status)
          .subscribe((user: any) => {
            this.messageService.add({
              severity: "info",
              summary: "Confirmed",
              detail: "Deshabilitado con éxito",
            });
            setTimeout(() => {
              this.getTeacherExpertAproved();
            }, 600);
