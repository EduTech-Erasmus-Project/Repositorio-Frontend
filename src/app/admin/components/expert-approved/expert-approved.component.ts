...
export class ExpertApprovedComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.get_ExpertAproved();
  }
  get_ExpertAproved(){
    this.administratorService.getExpertAproved().subscribe((user:any) => {
      this.isLoading=true;
  ...
  confirm2(event: Event,id:number,teacher_status:number,expert_status: number) {
    this.confirmationService.confirm({
        ...
        accept: () => {
          this.administratorService.updateCollaboratingExpertAproved(id,teacher_status,expert_status).subscribe((user:any) => {
