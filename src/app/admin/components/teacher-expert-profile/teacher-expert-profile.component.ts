  ...
  getProfile() {
    if (this.status === "pending") {
      this.administratorService
        .getTeacherAndExpertToAproveProfile(this.id)
        .subscribe((resp: any) => {
          this.isLoading = true;
          this.user = resp;
          ...
          this.rolesList.length > 1
            ? (this.rolText = "Roles solicitados por el usuario")
            : (this.rolText = "Rol solicitado por el usuario");
        });
    }
    if (this.status === "approved") {
      this.administratorService
        .getTeacherAndExpertprovedProfile(this.id)
        .subscribe((resp: any) => {
          this.isLoading = true;
          this.user = resp;
          this.expert = resp.collaboratingExpert;
          ...
          this.rolesList?.length > 1
            ? (this.rolText = "Roles aprobados")
            : (this.rolText = "Rol aprobado");
        });
    }
  }
}
