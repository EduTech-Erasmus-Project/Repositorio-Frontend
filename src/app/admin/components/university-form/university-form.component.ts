...
export class UniversityFormComponent implements OnInit {
  ...
  private async loadById() {
    try {
      let resp = await this.addressService.getUniversityById(this._id).toPromise();
  ...
  private async update(data) {
    return await this.addressService.updateUniversity(this._id, data).toPromise();
  }

  private async create(data) {
    return await this.addressService.createUniversity(data).toPromise();
  }
